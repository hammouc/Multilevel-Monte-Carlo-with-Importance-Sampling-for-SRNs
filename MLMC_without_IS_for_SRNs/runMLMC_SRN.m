function [cputime,Mean,Variance,samples,cost] = runMLMC_SRN(tol,L,L0,exp_number,target,tend)

% this code computes  the mean , the variance of MLMC estimator,  the number of needed samples
%as well the average cost per level   for a defined tolerance (tol)
...
%level to determine the optimla number of samples, or use opt below to have
%a new estimates 
%alternatively
%M0=10^6;
%samples= opt(tol,L,L0,M0,exp_number,target,tend);
...
%the variances at each level.  The sum of the variances must get below tol.
...
while (convergence==0)
   ...
   [m1,v1,m2,v2,md,vd,CPUTime]= coupled_estimator_ML(tend/2^(L-1),exp_number,target,sample_new,tend);
     mean_diff(1) = md;
    ...
    [m1,v1,m2,v2,md,vd,CPUTime]= coupled_estimator_ML(tend/2^(L-2),exp_number,target,sample_new,tend);
     mean_diff(2) = md;
...
for i = 3:(L-L0)
    ...
    [m1,v1,m2,v2,md,vd,CPUTime]= coupled_estimator_ML(tend/2^(L-i),exp_number,target,sample_new,tend);
     mean_diff(i) = md;
