...
% This code performs MLMC convergence tests (with importance sampling): saves in .txt file and plots 
% variances, expectations, kurtosis per  level for MLMC
...
% Request the user to enter importance sampling parameter,\delta, and return an error
% if not in (0,1)

IS_parameter = 'What is the value  of the importance sampling parameter? ';
delta=input(IS_parameter);
if delta>=1 || delta<=0
 error('Error: IS_parameter must be in (0,1)')
end 
...
for N1 = 1:10000:N
  ...
  elseif  exp==4
    if l==0
    Pf = single_estimator(hf,N2,exp,3);
    elseif l>0
